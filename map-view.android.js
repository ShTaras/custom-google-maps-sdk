            ...
            onMapReady: function (gMap) {
                ...
                gMap.setOnMarkerClickListener(new com.google.android.gms.maps.GoogleMap.OnMarkerClickListener({
                    onMarkerClick: function (gmsMarker) {
                        var marker = owner.findMarker(function (marker) { 
...
if (marker.android.getId) {
            ...
            return marker.title === gmsMarker.getTitle() && marker.snippet === gmsMarker.getSnippet() && marker.position.android.equals(gmsMarker.getPosition());
                ...
                gMap.setOnInfoWindowCloseListener(new com.google.android.gms.maps.GoogleMap.OnInfoWindowCloseListener({
                    onInfoWindowClose: function (gmsMarker) {
                        var marker = owner.findMarker(function (marker) { 
if (marker.android.getId) {
                return marker.android.getId() === gmsMarker.getId();
            }
            return marker.title === gmsMarker.getTitle() && marker.snippet === gmsMarker.getSnippet() && marker.position.android.equals(gmsMarker.getPosition());
                ...
                gMap.setOnIndoorStateChangeListener(new com.google.android.gms.maps.GoogleMap.OnIndoorStateChangeListener({
                    onIndoorBuildingFocused: function () {
                        var buildingFocused = gMap.getFocusedBuilding();
                        var data = null;
                        if (buildingFocused) {
                            var levels = [];
                            var count = 0;
                    ...
                    onIndoorLevelActivated: function (gmsIndoorBuilding) {
                        var level = gmsIndoorBuilding.getLevels().get(gmsIndoorBuilding.getActiveLevelIndex());
